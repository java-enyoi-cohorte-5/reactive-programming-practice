...
01 `ex01-mono-just-or-empty` - `switchIfEmpty` para defaults
02 `ex02-flux-map-filter` - limpieza con `map` y `filter`
03 `ex03-mono-flatmap-chain` - composicion con `flatMap`
04 `ex04-flux-filter-non-empty` - filtrado declarativo
05 `ex05-mono-on-error-resume` - fallback con `onErrorResume`
06 `ex06-flux-distinct-stable` - deduplicacion estable
07 `ex07-mono-default-if-empty` - defaults con `defaultIfEmpty`
08 `ex08-flux-concat-values` - secuencias con `concatWithValues`
09 `ex09-mono-map-transform` - transformaciones puras
